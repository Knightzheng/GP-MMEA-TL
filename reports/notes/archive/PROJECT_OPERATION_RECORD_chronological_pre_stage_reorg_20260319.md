...
## 30. 2026-03-15 追加记录（主线材料完整性自动校验）

本次追加操作：

1. 新增主线材料完整性校验脚本：
   - `scripts/verify_mainline_artifacts.py`
2. 将“当前主线仓库是否仍保持完整和干净”这件事，收敛成一个可重复执行的自动检查入口。
3. 校验范围包括：
   - 主线 README / notes / transfer 指南文件；
   - 四个目标域正式 baseline / method run 根目录；
   - run 级别关键留痕文件是否齐全；
   - `8` 个案例样本与 GPU 图表友好数据是否存在；
   - `H3` 相关脚本、目录与结果是否保持移除状态。
4. 自动生成：
...
1. 当前自动校验共覆盖 `38` 项检查，结果为 `0` 失败。
2. 说明当前仓库中的主线正式入口、正式 run 留痕、案例 / GPU 补强材料，以及 H3 清理状态，与当前项目口径保持一致。
3. 这份校验报告不替代论文表述，但非常适合作为后续继续整理、答辩前自查和仓库健康检查的统一入口。

本次追加的直接作用：

1. 后续不再需要人工逐目录确认“正式 run 还在不在”“案例和 GPU 补强文件有没有丢”“H3 是否又被误放回主线目录”。
2. 让项目当前的“主线已闭环 + 辅助项边界清楚”不只是文档口径，还具备了自动可验证的仓库状态检查能力。

## 31. 2026-03-16 追加记录（论文终稿整合方向收口）

本次追加操作：

1. 重新核对共享文件当前状态，并从“继续补实验”与“继续做终稿整合”两条路径中重新评估下一步最值得投入的方向。
2. 明确判断：当前最优先的下一步不再是新实验，而是把现有主线结果、辅助补强、边界说明和答辩材料路径整合成论文终稿可直接吸收的统一方案。
3. 新增：
   - `reports/notes/thesis_final_integration_packet_20260316.md`

关键判断：

1. 主线实验已经闭环，继续补实验的边际收益已经低于整合现有材料的收益。
2. 当前论文线程最需要的不是更多零散文件，而是一份按第四章、第五章、附录和答辩组织起来的吸收路径说明。
3. 因此，项目优化线程当前应继续坚持：
   - 不重启 `H3`；
   - 不为额外 baseline 重开高成本 rerun；
   - 不再把辅助支撑项写成主线完成前提。

本次追加的直接作用：

1. 让论文线程可以更快把已有项目材料转化为终稿结构，而不是继续在多个结果文件间来回跳转。
2. 让后续项目优化工作继续围绕“提高论文终稿吸收效率”而不是“继续加实验”展开。

## 32. 2026-03-16 追加记录（目录结构导航优化）

本次追加操作：

1. 为 `reports/notes/` 新增目录级导航：
   - `reports/notes/README.md`
2. 为 `scripts/` 新增目录级导航：
   - `scripts/README.md`
3. 为 `runs/experiments/` 新增目录级导航：
...
1. 当前项目真正需要的结构优化，不是物理搬动正式结果文件，而是补足目录级导航与边界说明。
2. 这样做的收益更高，因为它不会破坏现有正式结果路径、脚本引用和论文线程已吸收的入口。
3. `reports/notes/`、`scripts/` 和 `runs/experiments/` 是当前理解成本最高、但又最适合用 README 收口的三个目录。

本次追加的直接作用：

1. 降低后来者在 `notes/`、`scripts/`、`runs/experiments/` 三个目录中的跳转和误判成本。
2. 让“当前正式主线入口”和“历史探索留痕”之间的边界更清楚。
3. 在不扰动正式结果文件路径的前提下，进一步优化了项目文件结构。

## 33. 2026-03-16 追加记录（终稿 / 答辩浓缩材料包）

本次追加操作：

1. 重新检查共享文件最新状态，吸收论文线程在 `6.16` 中提出的新增要求。
2. 明确选择当前最推荐的下一步方向：
   - 不再继续补高成本实验；
   - 优先补“终稿和答辩可直接吸收的浓缩材料”。
3. 新增：
...
1. 当前主线实验已经闭环，继续补实验的边际收益已低于整理现有证据链的收益。
2. 共享文件最新提出的三项需求，都是“浓缩已有材料”，而不是“新增实验结论”，因此非常适合作为当前项目优化线程的下一步。
3. `fr_en` 目前没有单独保留代表性案例，这不是主线缺失，而是说明该目标域当前主要由正式 `5-seed` 主表和显著性支撑。

本次追加的直接作用：

1. 让论文线程能直接拿到一页式“四目标域主证据地图”。
2. 让答辩时常见的范围、GPU、案例、额外 baseline 和 `H3` 追问有统一口径。
3. 让现有 `8` 个正式案例不只是散列样本，而是被组织成几类清晰的成功 / 失败模式。

## 34. 2026-03-19 追加记录（中期材料清理与事实源回收）

本次追加操作：

1. 复核学校中期报告模板、填写说明、开题报告与当前项目主线结果。
2. 删除上一线程自动生成的中期填写稿、指导记录草稿、教师评语稿和模板导出成品，避免未核实内容继续污染项目目录。
3. 重写 `reports/midterm/README.md`，将中期目录改为“模板 + 开题报告 + 真实结果文件”的事实源导航。
4. 新增 `reports/midterm/rendered/README.md`，明确 `rendered/` 目录只保留学校模板原件，不再保留自动生成成品。

关键判断：

1. 中期报告必须对照开题报告阶段计划和项目真实结果撰写，不能沿用旧自动稿。
2. 中期目录中凡是未经人工核对、且无法由当前仓库结果直接支撑的内容，都应从当前工作区移除。
3. 历史草稿可作为线索保留，但必须显式标记为“不可直接提交”。

本次追加的直接作用：

1. 重新建立了中期报告可用资料的边界。
2. 降低了后续写作时误引旧自动稿和旧导出成品的风险。
3. 为后续基于真实结果重新撰写中期报告提供了干净起点。
