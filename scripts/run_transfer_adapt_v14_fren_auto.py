...
def run_variant(seed: int, target_config: str, variant_name: str, runner_python: str, meaformer_python: str, stage_root: str):
    cmd = [
        ...
        target_config,
        "--tag",
        f"tmmeada_transfer_adapt_v14_fren_{variant_name}_s{seed}",
        "--stage-root",
        stage_root,
        "--seed",
        str(seed),
...
def summarize_vs_ref(runner_python: str, ref_target_dir: str, tmmeada_target_dir: str, prefix: str):
    run_cmd(
        [
            ...
            tmmeada_target_dir,
            "--baseline-out",
            f"{prefix}_v13_ref_summary.csv",
            "--tmmeada-out",
            f"{prefix}_tmmeada_summary.csv",
            "--compare-out-csv",
            f"{prefix}_compare_vs_v13.csv",
            "--compare-out-md",
            f"{prefix}_compare_vs_v13.md",
        ]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Auto optimize transfer-adapt on fr_en v14: earlier IL refresh strategies."
    )
    ...
    variants = [
        {
            "name": "v14a_refresh5_da0025",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v14a_refresh5_da0025.yaml",
        },
        {
            "name": "v14b_refresh4_da0025",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v14b_refresh4_da0025.yaml",
        },
        {
            "name": "v14c_refresh5_da0030",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v14c_refresh5_da0030.yaml",
        },
    ]

    decision_rows = []
    for v in variants:
        stage_root = f"transfer/transfer_adapt_v14_fren_pilot_{v['name']}"
        run_variant(
        ...
        prefix = f"reports/transfer/transfer_adapt_v14_fren_pilot_{v['name']}"
        summarize(
            runner_python=args.runner_python,
    ...
    if not valid:
        raise RuntimeError("No valid fr_en pilot result for v14.")
    best = sorted(valid, key=lambda x: x["delta_avg_mrr_mean_vs_baseline"], reverse=True)[0]

    formal_stage_root = f"transfer/transfer_adapt_v14_fren_formal_{best['name']}"
    run_variant(
    ...
    if formal_run_dir is None:
        raise RuntimeError("Formal fr_en run not found for v14.")
    ...
    merged_target_root = (
        Path("runs")
        / "transfer"
        / f"transfer_adapt_v14_fren_2seed_{best['name']}"
        / "target_eval"
    )
    copy_run_dir(pilot_run_dir, merged_target_root)
    copy_run_dir(formal_run_dir, merged_target_root)

    final_prefix = "reports/transfer/transfer_adapt_v14_fren_2seed"
    summarize(
        runner_python=args.runner_python,
    ...
    summarize_vs_ref(
        runner_python=args.runner_python,
        ref_target_dir="runs/transfer/transfer_adapt_v13_fren_2seed_v13a_source_select_mild/target_eval",
        tmmeada_target_dir=str(merged_target_root),
        prefix=final_prefix,
    )
    ...
    payload = {
        ...
        "final_compare_vs_v13_csv": f"{final_prefix}_compare_vs_v13.csv",
    }
    out_json = Path("reports/transfer/transfer_adapt_v14_fren_decision.json")
    out_md = Path("reports/transfer/transfer_adapt_v14_fren_decision.md")
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "# Transfer Adapt v14 fr_en Decision",
        "",
    ...
    lines.extend(
        [
            ...
            f"- `{final_prefix}_compare_vs_v13.csv`",
        ]
    )
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
