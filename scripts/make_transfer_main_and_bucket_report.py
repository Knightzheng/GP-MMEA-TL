...
SOURCE_SPECS = [
    ...
    },
    {
        "target": "FBYG15K",
        "scenario": "cross_graph",
        "method_variant": "v24b_strictsrc_staged_fresh_il_top400_expand5",
        "source_csv_candidates": [
            REPORT_DIR / "transfer_adapt_v24_fbyg_v24b_expand5_compare_vs_baseline.csv",
...
def write_main_md(rows: List[Dict[str, object]], out_md: Path) -> None:
    ...
    lines.append("- `FBYG15K` now uses the refreshed `v24b_strictsrc_staged_fresh_il_top400_expand5` result.")
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
