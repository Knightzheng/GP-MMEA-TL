...
VARIANTS = {
    "v16a": {
        "config": "configs/transfer_adapt/tmmeada_target_fbdb15k_v16a_refresh4_balanced.yaml",
        "stage_root": "transfer/transfer_adapt_v16_fbdb_pilot_v16a",
        "tag_prefix": "tmmeada_transfer_adapt_v16_fbdb_v16a",
        "pilot_ref_dir": ROOT / "runs/transfer/transfer_adapt_v16_fbdb_v16a_pilot_ref/target_eval",
        "full_ref_dir": ROOT / "runs/transfer/transfer_adapt_v16_fbdb_v16a_expand5_ref/target_eval",
    },
    "v16b": {
        "config": "configs/transfer_adapt/tmmeada_target_fbdb15k_v16b_refresh4_strict.yaml",
        "stage_root": "transfer/transfer_adapt_v16_fbdb_pilot_v16b",
        "tag_prefix": "tmmeada_transfer_adapt_v16_fbdb_v16b",
        "pilot_ref_dir": ROOT / "runs/transfer/transfer_adapt_v16_fbdb_v16b_pilot_ref/target_eval",
        "full_ref_dir": ROOT / "runs/transfer/transfer_adapt_v16_fbdb_v16b_expand5_ref/target_eval",
    },
    "v16c": {
        "config": "configs/transfer_adapt/tmmeada_target_fbdb15k_v16c_refresh5_srcsel.yaml",
        "stage_root": "transfer/transfer_adapt_v16_fbdb_pilot_v16c",
        "tag_prefix": "tmmeada_transfer_adapt_v16_fbdb_v16c",
        "pilot_ref_dir": ROOT / "runs/transfer/transfer_adapt_v16_fbdb_v16c_pilot_ref/target_eval",
        "full_ref_dir": ROOT / "runs/transfer/transfer_adapt_v16_fbdb_v16c_expand5_ref/target_eval",
    },
}
...
def main():
    parser = argparse.ArgumentParser(description="Iterate FBDB15K transfer-adapt variants with resumable summaries.")
    parser.add_argument(
        "--run-missing",
        type=int,
        default=1,
    ...
    baseline_pilot_all_ref = ROOT / "runs/transfer/transfer_adapt_v16_fbdb_pilot_baseline_ref/target_eval"
    ...
    for key in ["v16a", "v16b", "v16c"]:
        ...
        compare_csv = None
        delta = None
        baseline_variant_ref = ROOT / "runs/transfer" / f"transfer_adapt_v16_fbdb_{key}_pilot_baseline_matched_ref" / "target_eval"
        if selected_seeds:
        ...
        if baseline_pilot_selected and selected:
            compare_csv_path = summarize_compare(
                prefix=f"transfer_adapt_v16_fbdb_pilot_{key}",
    ...
    if should_expand:
        ...
        if best_selected_seeds:
            best_baseline_ref = ROOT / "runs/transfer" / f"transfer_adapt_v16_fbdb_{best_variant}_expand5_baseline_matched_ref" / "target_eval"
        ...
        if best_baseline_selected and best_selected:
            compare_csv = summarize_compare(
                prefix=f"transfer_adapt_v16_fbdb_{best_variant}_expand5",
    ...
    out_json = ROOT / "reports/transfer/transfer_adapt_v16_fbdb_iter_decision.json"
    out_md = ROOT / "reports/transfer/transfer_adapt_v16_fbdb_iter_decision.md"
    out_json.write_text(json.dumps(records, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "# FBDB15K v16 Iteration Decision",
        "",
        ...
        "## Pilot Summary",
        "",
        "| variant | delta_avg_mrr_mean | selected_seeds |",
        "|---|---:|---|",
    ]
    for key in ["v16a", "v16b", "v16c"]:
