...
def run_variant(seed: int, target_config: str, variant_name: str, runner_python: str, meaformer_python: str, stage_root: str):
    cmd = [
        ...
        target_config,
        "--tag",
        f"tmmeada_transfer_adapt_v12_fren_{variant_name}_s{seed}",
        "--stage-root",
        stage_root,
        "--seed",
        str(seed),
...
def summarize_vs_ref(runner_python: str, ref_target_dir: str, tmmeada_target_dir: str, prefix: str):
    run_cmd(
        [
            ...
            tmmeada_target_dir,
            "--baseline-out",
            f"{prefix}_v10_ref_summary.csv",
            "--tmmeada-out",
            f"{prefix}_tmmeada_summary.csv",
            "--compare-out-csv",
            f"{prefix}_compare_vs_v10.csv",
            "--compare-out-md",
            f"{prefix}_compare_vs_v10.md",
        ]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Auto optimize transfer-adapt on fr_en v12: recover+high-keep mild filters."
    )
    ...
    variants = [
        {
            "name": "v12a_recover_v10",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v12a_recover_v10.yaml",
        },
        {
            "name": "v12b_mild_filter_highkeep",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v12b_mild_filter_highkeep.yaml",
        },
        {
            "name": "v12c_mild_filter_da03",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v12c_mild_filter_da03.yaml",
        },
    ]

    decision_rows = []
    for v in variants:
        stage_root = f"transfer/transfer_adapt_v12_fren_pilot_{v['name']}"
        run_variant(
        ...
        prefix = f"reports/transfer/transfer_adapt_v12_fren_pilot_{v['name']}"
        summarize(
            runner_python=args.runner_python,
    ...
    if not valid:
        raise RuntimeError("No valid fr_en pilot result for v12.")
    best = sorted(valid, key=lambda x: x["delta_avg_mrr_mean_vs_baseline"], reverse=True)[0]

    formal_stage_root = f"transfer/transfer_adapt_v12_fren_formal_{best['name']}"
    run_variant(
    ...
    if formal_run_dir is None:
        raise RuntimeError("Formal fr_en run not found for v12.")
    ...
    merged_target_root = (
        Path("runs")
        / "transfer"
        / f"transfer_adapt_v12_fren_2seed_{best['name']}"
        / "target_eval"
    )
    copy_run_dir(pilot_run_dir, merged_target_root)
    copy_run_dir(formal_run_dir, merged_target_root)

    final_prefix = "reports/transfer/transfer_adapt_v12_fren_2seed"
    summarize(
        runner_python=args.runner_python,
    ...
    summarize_vs_ref(
        runner_python=args.runner_python,
        ref_target_dir="runs/transfer/transfer_adapt_v10_fren_2seed_v10b_da0025/target_eval",
        tmmeada_target_dir=str(merged_target_root),
        prefix=final_prefix,
    )
    ...
    payload = {
        ...
        "final_compare_vs_v10_csv": f"{final_prefix}_compare_vs_v10.csv",
    }
    out_json = Path("reports/transfer/transfer_adapt_v12_fren_decision.json")
    out_md = Path("reports/transfer/transfer_adapt_v12_fren_decision.md")
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "# Transfer Adapt v12 fr_en Decision",
        "",
    ...
    lines.extend(
        [
            ...
            f"- `{final_prefix}_compare_vs_v10.csv`",
        ]
    )
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
