...
def run_one(seed: int, runner_python: str, meaformer_python: str, stage_root: str):
    ...
    tmmeada_ckpt = resolve_source_model_name(seed=seed)
    tmmeada_cmd = [
        runner_python,
        ...
        tmmeada_targets,
        "--tag",
        f"tmmeada_transfer_adapt_tuned_lite_s{seed}",
        "--stage-root",
        stage_root,
        "--seed",
        str(seed),
...
def main():
    parser = argparse.ArgumentParser(
        description="Run transfer adapt tuned-lite queue on ja_en and FBDB15K (TMMEA-DA only)."
    )
    ...
    parser.add_argument("--stage-root", default="transfer/transfer_adapt_tuned")
    ...
    print("[DONE] transfer adapt tuned-lite queue completed.")
