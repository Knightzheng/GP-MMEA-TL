...
def run_variant(seed: int, variant: dict, runner_python: str, meaformer_python: str, stage_root: str, tag: str):
    cmd = [
        runner_python,
        "scripts/run_transfer_train_eval.py",
        "--source-config",
        variant["source_config"],
        "--target-configs",
        variant["target_config"],
        "--tag",
        tag,
        "--stage-root",
        stage_root,
    ...
    ckpt = resolve_source_model_name(seed=seed, tmmeada=variant["tmmeada_source"])
    if ckpt:
...
def summarize_vs_ref(runner_python: str, ref_target_dir: str, tmmeada_target_dir: str, prefix: str):
    run_cmd(
        [
            ...
            tmmeada_target_dir,
            "--baseline-out",
            f"{prefix}_v8_ref_summary.csv",
            "--tmmeada-out",
            f"{prefix}_tmmeada_summary.csv",
            "--compare-out-csv",
            f"{prefix}_compare_vs_v8.csv",
            "--compare-out-md",
            f"{prefix}_compare_vs_v8.md",
        ]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Auto optimize transfer-adapt on fr_en: pilot select best -> formal 2-seed compare."
    )
    ...
    variants = [
        {
            "name": "v9a_tm_src_mild_da",
            "source_config": "configs/transfer/tmmeada_source_zh_en_epoch10.yaml",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v9a_mild_da_unsup_il.yaml",
            "tmmeada_source": True,
        },
        {
            "name": "v9b_base_src_mild_da",
            "source_config": "configs/transfer/meaformer_source_zh_en_epoch10.yaml",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v9b_mild_da_unsup_il.yaml",
            "tmmeada_source": False,
        },
    ]

    decision_rows = []
    for v in variants:
        stage_root = f"transfer/transfer_adapt_v9_fren_pilot_{v['name']}"
        tag = f"tmmeada_transfer_adapt_v9_fren_pilot_{v['name']}_s{args.pilot_seed}"
        run_variant(
        ...
        prefix = f"reports/transfer/transfer_adapt_v9_fren_pilot_{v['name']}"
        summarize(
            runner_python=args.runner_python,
        ...
        decision_rows.append(
            {
                ...
                "tmmeada_source": v["tmmeada_source"],
    ...
    if best_variant is None:
        raise RuntimeError("Best variant not found in variants list.")

    formal_stage_root = f"transfer/transfer_adapt_v9_fren_formal_{best_variant['name']}"
    formal_tag = f"tmmeada_transfer_adapt_v9_fren_formal_{best_variant['name']}_s{args.formal_seed}"
    run_variant(
    ...
    merged_target_root = (
        Path("runs")
        / "transfer"
        / f"transfer_adapt_v9_fren_2seed_{best_variant['name']}"
        / "target_eval"
    )
    copy_run_dir(pilot_run_dir, merged_target_root)
    copy_run_dir(formal_run_dir, merged_target_root)

    final_prefix = "reports/transfer/transfer_adapt_v9_fren_2seed"
    summarize(
        runner_python=args.runner_python,
    ...
    summarize_vs_ref(
        runner_python=args.runner_python,
        ref_target_dir="runs/transfer/transfer_adapt_v8_expand_2seed_tmmeada/target_eval",
        tmmeada_target_dir=str(merged_target_root),
        prefix=final_prefix,
    )
    ...
    payload = {
        ...
        "best_variant_source_config": best_variant["source_config"],
        "best_variant_target_config": best_variant["target_config"],
        ...
        "final_compare_vs_v8_csv": f"{final_prefix}_compare_vs_v8.csv",
    }
    out_json = Path("reports/transfer/transfer_adapt_v9_fren_decision.json")
    out_md = Path("reports/transfer/transfer_adapt_v9_fren_decision.md")
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "# Transfer Adapt v9 fr_en Decision",
        "",
        f"- pilot_seed: `{args.pilot_seed}`",
        f"- formal_seed: `{args.formal_seed}`",
        f"- best_variant: `{best_variant['name']}`",
    ...
    lines.extend(
        [
            ...
            f"- `{final_prefix}_compare_vs_v8.csv`",
        ]
    )
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
