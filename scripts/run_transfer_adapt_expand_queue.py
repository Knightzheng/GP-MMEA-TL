...
def run_one(seed: int, runner_python: str, meaformer_python: str, stage_root: str):
    ...
    baseline_cmd = [
        ...
        baseline_targets,
        "--tag",
        f"baseline_transfer_adapt_expand_s{seed}",
        "--stage-root",
        stage_root,
        "--seed",
        str(seed),
    ...
    if baseline_ckpt:
        baseline_cmd.extend(["--source-model-name", baseline_ckpt])
    ...
    tmmeada_cmd = [
        ...
        tmmeada_targets,
        "--tag",
        f"tmmeada_transfer_adapt_expand_s{seed}",
        "--stage-root",
        f"{stage_root}_tmmeada",
        "--seed",
        str(seed),
        "--runner-python",
        meaformer_python,
    ...
    if tmmeada_ckpt:
        tmmeada_cmd.extend(["--source-model-name", tmmeada_ckpt])
...
def main():
    parser = argparse.ArgumentParser(
        description="Run transfer adapt expansion queue on fr_en and FBYG15K."
    )
    ...
    parser.add_argument("--stage-root", default="transfer/transfer_adapt_expand")
    ...
    print("[DONE] transfer adapt expansion queue completed.")
