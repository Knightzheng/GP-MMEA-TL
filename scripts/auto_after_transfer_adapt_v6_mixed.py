...
def collect_latest_status(target_eval_dir: Path) -> Dict[Tuple[int, str], Dict]:
    ...
    for run_dir in target_eval_dir.iterdir():
        ...
        if prev is None or mtime > prev["mtime"]:
            result[key] = {"done": done, "mtime": mtime}
    return result
...
def wait_for_done(
    ...
    while True:
        ...
        if not missing:
            print(f"[AUTO-V6] v6 queue done at {now_str()}")
            return
        if time.time() > deadline:
            raise TimeoutError(f"Timeout waiting v6 done: {missing}")
        print(f"[AUTO-V6] waiting {now_str()} | missing={missing}")
        time.sleep(poll_seconds)


def main():
    parser = argparse.ArgumentParser(description="Wait transfer_adapt_v6_mixed queue then auto summarize.")
    parser.add_argument("--runner-python", default=r"D:\Anaconda_envs\envs\bysj-main\python.exe")
    parser.add_argument("--target-dir", default="runs/transfer/transfer_adapt_v6_mixed/target_eval")
    ...
    run_cmd(
        [
            ...
            "--baseline-out",
            "reports/transfer/transfer_adapt_v6_mixed_baseline_ref_summary.csv",
            "--tmmeada-out",
            "reports/transfer/transfer_adapt_v6_mixed_tmmeada_summary.csv",
            "--compare-out-csv",
            "reports/transfer/transfer_adapt_v6_mixed_compare_vs_baseline.csv",
            "--compare-out-md",
            "reports/transfer/transfer_adapt_v6_mixed_compare_vs_baseline.md",
        ]
    )
    run_cmd(
        [
            ...
            "scripts/summarize_transfer_formal.py",
            "--baseline-target-dir",
            "runs/transfer/transfer_adapt_v5/target_eval",
            "--tmmeada-target-dir",
            "runs/transfer/transfer_adapt_v6_mixed/target_eval",
            "--baseline-out",
            "reports/transfer/transfer_adapt_v6_mixed_v5_ref_summary.csv",
            "--tmmeada-out",
            "reports/transfer/transfer_adapt_v6_mixed_tmmeada_summary.csv",
            "--compare-out-csv",
            "reports/transfer/transfer_adapt_v6_mixed_compare_vs_v5.csv",
            "--compare-out-md",
            "reports/transfer/transfer_adapt_v6_mixed_compare_vs_v5.md",
        ]
    )
    print("[AUTO-V6] done.")
