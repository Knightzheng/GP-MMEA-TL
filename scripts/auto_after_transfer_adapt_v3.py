...
def wait_for_done(
    ...
    while True:
        ...
        if not missing:
            print(f"[AUTO-V3] v3 queue done at {now_str()}")
            return
        if time.time() > deadline:
            raise TimeoutError(f"Timeout waiting v3 done: {missing}")
        print(f"[AUTO-V3] waiting {now_str()} | missing={missing}")
        time.sleep(poll_seconds)


def main():
    parser = argparse.ArgumentParser(description="Wait transfer_adapt_v3 queue then auto summarize.")
    parser.add_argument("--runner-python", default=r"D:\Anaconda_envs\envs\bysj-main\python.exe")
    parser.add_argument("--target-dir", default="runs/transfer/transfer_adapt_v3/target_eval")
    ...
    run_cmd(
        [
            ...
            "--baseline-out",
            "reports/transfer/transfer_adapt_v3_baseline_ref_summary.csv",
            "--tmmeada-out",
            "reports/transfer/transfer_adapt_v3_tmmeada_summary.csv",
            "--compare-out-csv",
            "reports/transfer/transfer_adapt_v3_compare_vs_baseline.csv",
            "--compare-out-md",
            "reports/transfer/transfer_adapt_v3_compare_vs_baseline.md",
        ]
    )
    run_cmd(
        [
            ...
            "runs/transfer/transfer_adapt_v3/target_eval",
            "--baseline-out",
            "reports/transfer/transfer_adapt_v3_tuned_lite_ref_summary.csv",
            "--tmmeada-out",
            "reports/transfer/transfer_adapt_v3_tmmeada_summary.csv",
            "--compare-out-csv",
            "reports/transfer/transfer_adapt_v3_compare_vs_tuned_lite.csv",
            "--compare-out-md",
            "reports/transfer/transfer_adapt_v3_compare_vs_tuned_lite.md",
        ]
    )
    print("[AUTO-V3] done.")
