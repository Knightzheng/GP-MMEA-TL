...
BASELINE_MERGED_TARGET_EVAL = ROOT / "runs/transfer/transfer_adapt_fbdb_expand5_merged_baseline/target_eval"
CURRENT_BEST_COMPARE_CSV = ROOT / "reports/transfer/transfer_adapt_v7_fbdb_expand5_compare_vs_baseline.csv"
...
SOURCE_MODELS = {
    ...
    2026: "MEAformer_DBP15K_zh_en_transfer_src_zh_en_epoch10_baseline_transfer_adapt_FBDB15K_expand5_s2026_src_s2026_",
    7: "MEAformer_DBP15K_zh_en_transfer_src_zh_en_epoch10_baseline_transfer_adapt_FBDB15K_expand5_s7_src_s7_",
    123: "MEAformer_DBP15K_zh_en_transfer_src_zh_en_epoch10_baseline_transfer_adapt_FBDB15K_expand5_s123_src_s123_",
}

VARIANTS = {
    "v18a": {
        "config": "configs/transfer_adapt/tmmeada_target_fbdb15k_v18a_bipartite_no_il.yaml",
        "stage_root": "transfer/transfer_adapt_v18_fbdb_pilot_v18a",
        "tag_prefix": "tmmeada_transfer_adapt_v18_fbdb_v18a",
        "pilot_ref_dir": ROOT / "runs/transfer/transfer_adapt_v18_fbdb_v18a_pilot_ref/target_eval",
        "full_ref_dir": ROOT / "runs/transfer/transfer_adapt_v18_fbdb_v18a_expand5_ref/target_eval",
        "transfer_skip_keys": "multimodal_encoder.entity_emb.weight",
    },
    "v18b": {
        "config": "configs/transfer_adapt/tmmeada_target_fbdb15k_v18b_bipartite_late_il.yaml",
        "stage_root": "transfer/transfer_adapt_v18_fbdb_pilot_v18b",
        "tag_prefix": "tmmeada_transfer_adapt_v18_fbdb_v18b",
        "pilot_ref_dir": ROOT / "runs/transfer/transfer_adapt_v18_fbdb_v18b_pilot_ref/target_eval",
        "full_ref_dir": ROOT / "runs/transfer/transfer_adapt_v18_fbdb_v18b_expand5_ref/target_eval",
        "transfer_skip_keys": "multimodal_encoder.entity_emb.weight",
    },
    "v18c": {
        "config": "configs/transfer_adapt/tmmeada_target_fbdb15k_v18c_bipartite_late_il_skiprel.yaml",
        "stage_root": "transfer/transfer_adapt_v18_fbdb_pilot_v18c",
        "tag_prefix": "tmmeada_transfer_adapt_v18_fbdb_v18c",
        "pilot_ref_dir": ROOT / "runs/transfer/transfer_adapt_v18_fbdb_v18c_pilot_ref/target_eval",
        "full_ref_dir": ROOT / "runs/transfer/transfer_adapt_v18_fbdb_v18c_expand5_ref/target_eval",
        "transfer_skip_keys": (
...
def main():
    parser = argparse.ArgumentParser(description="Iterate FBDB15K v18 bipartite-seed variants with resumable summaries.")
    parser.add_argument(
        "--run-missing",
        type=int,
        default=1,
    ...
    baseline_pilot_all_ref = ROOT / "runs/transfer/transfer_adapt_v18_fbdb_pilot_baseline_ref/target_eval"
    ...
    for key in ["v18a", "v18b", "v18c"]:
        ...
        compare_csv = None
        delta = None
        baseline_variant_ref = ROOT / "runs/transfer" / f"transfer_adapt_v18_fbdb_{key}_pilot_baseline_matched_ref" / "target_eval"
        if selected_seeds:
        ...
        if baseline_pilot_selected and selected:
            compare_csv_path = summarize_compare(
                prefix=f"transfer_adapt_v18_fbdb_pilot_{key}",
    ...
    if should_expand:
        ...
        if best_selected_seeds:
            best_baseline_ref = ROOT / "runs/transfer" / f"transfer_adapt_v18_fbdb_{best_variant}_expand5_baseline_matched_ref" / "target_eval"
        ...
        if best_baseline_selected and best_selected:
            compare_csv = summarize_compare(
                prefix=f"transfer_adapt_v18_fbdb_{best_variant}_expand5",
    ...
    out_json = ROOT / "reports/transfer/transfer_adapt_v18_fbdb_iter_decision.json"
    out_md = ROOT / "reports/transfer/transfer_adapt_v18_fbdb_iter_decision.md"
    ...
    lines = [
        ...
        f"- reference_delta_avg_mrr_mean(v7_expand5): `{ref_delta}`",
    ...
    for key in ["v18a", "v18b", "v18c"]:
        ...
        lines.append(
            f"| {key} | {item.get('delta_avg_mrr_mean')} | {item.get('selected_seeds')} | {item.get('transfer_skip_keys')} |"
        )
    lines.append("")
