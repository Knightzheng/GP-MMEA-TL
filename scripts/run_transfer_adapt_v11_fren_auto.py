...
def run_variant(seed: int, target_config: str, variant_name: str, runner_python: str, meaformer_python: str, stage_root: str):
    cmd = [
        ...
        target_config,
        "--tag",
        f"tmmeada_transfer_adapt_v11_fren_{variant_name}_s{seed}",
        "--stage-root",
        stage_root,
        "--seed",
        str(seed),
...
def main():
    parser = argparse.ArgumentParser(
        description="Auto optimize transfer-adapt on fr_en v11 with confidence-filter variants."
    )
    ...
    variants = [
        {
            "name": "v11a_strict_filter",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v11a_strict_filter.yaml",
        },
        {
            "name": "v11b_balanced_filter",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v11b_balanced_filter.yaml",
        },
        {
            "name": "v11c_loose_filter",
            "target_config": "configs/transfer_adapt/tmmeada_target_fr_en_v11c_loose_filter.yaml",
        },
    ]

    decision_rows = []
    for v in variants:
        stage_root = f"transfer/transfer_adapt_v11_fren_pilot_{v['name']}"
        run_variant(
        ...
        prefix = f"reports/transfer/transfer_adapt_v11_fren_pilot_{v['name']}"
        summarize(
            runner_python=args.runner_python,
    ...
    if not valid:
        raise RuntimeError("No valid fr_en pilot result for v11.")
    best = sorted(valid, key=lambda x: x["delta_avg_mrr_mean_vs_baseline"], reverse=True)[0]

    formal_stage_root = f"transfer/transfer_adapt_v11_fren_formal_{best['name']}"
    run_variant(
    ...
    if formal_run_dir is None:
        raise RuntimeError("Formal fr_en run not found for v11.")
    ...
    merged_target_root = (
        Path("runs")
        / "transfer"
        / f"transfer_adapt_v11_fren_2seed_{best['name']}"
        / "target_eval"
    )
    copy_run_dir(pilot_run_dir, merged_target_root)
    copy_run_dir(formal_run_dir, merged_target_root)

    final_prefix = "reports/transfer/transfer_adapt_v11_fren_2seed"
    summarize(
        runner_python=args.runner_python,
    ...
    out_json = Path("reports/transfer/transfer_adapt_v11_fren_decision.json")
    out_md = Path("reports/transfer/transfer_adapt_v11_fren_decision.md")
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "# Transfer Adapt v11 fr_en Decision",
        "",
